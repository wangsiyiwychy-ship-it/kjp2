...
def search_bacteria():
    try:
        ...
        # 查找匹配的细菌
        matched_bacteria = None
        for bacteria in antibiotic_data:
            if name in bacteria['name'].lower():
                matched_bacteria = bacteria
                break
        
        if matched_bacteria:
            # 提取抗生素数据
            antibiotics = []
            for drug_id, drug_name in drug_mapping.items():
                if str(drug_id) in matched_bacteria['antibiotics']:
                    antibiotics.append({
                        'name': drug_name,
                        'id': drug_id,
                        'result': matched_bacteria['antibiotics'][str(drug_id)]
                    })
            
            return jsonify({
                'success': True,
                'bacteria': matched_bacteria['name'],
                'antibiotics': antibiotics
            })
        else:
            return jsonify({'success': False, 'message': '未找到匹配的细菌'}), 404
    except Exception as e:
        app.logger.error(f"搜索细菌错误: {str(e)}")
        return jsonify({'success': False, 'error': '搜索处理错误'}), 500
...
def search_drug():
    try:
        ...
        if matched_drug_id:
            ...
            bacteria_results = []
            for bacteria in antibiotic_data:
                if str(matched_drug_id) in bacteria['antibiotics']:
                    bacteria_results.append({
                        'name': bacteria['name'],
                        'result': bacteria['antibiotics'][str(matched_drug_id)]
                    })
            ...
            return jsonify({'success': False, 'message': '未找到匹配的药物'}), 404
    except Exception as e:
        app.logger.error(f"搜索药物错误: {str(e)}")
        return jsonify({'success': False, 'error': '搜索处理错误'}), 500
...
def compare_bacteria():
    try:
        ...
        if not bacteria_names:
            return jsonify({'success': False, 'error': '请指定要比较的细菌'}), 400
        
        # 收集每个细菌的数据
        results = []
        for name in bacteria_names:
            for bacteria in antibiotic_data:
                if name.lower() == bacteria['name'].lower():
                    results.append({
                        'bacteria': bacteria['name'],
                        'bacteria_results': bacteria['antibiotics']
                    })
                    break
        
        return jsonify({'success': True, 'data': results})
    except Exception as e:
        app.logger.error(f"比较细菌错误: {str(e)}")
        return jsonify({'success': False, 'error': '比较处理错误'}), 500
...
def compare_drug():
    try:
        ...
        if not drug_names:
            return jsonify({'success': False, 'error': '请指定要比较的药物'}), 400
        
        # 创建药物名称到ID的映射
        drug_ids = {}
        for drug_name in drug_names:
            for drug_id, name in drug_mapping.items():
        ...
        for bacteria in antibiotic_data:
            ...
            for drug_name, drug_id in drug_ids.items():
                if str(drug_id) in bacteria['antibiotics']:
                    drug_results[drug_name] = bacteria['antibiotics'][str(drug_id)]
                    found = True
        ...
        app.logger.error(f"比较药物错误: {str(e)}")
        return jsonify({'success': False, 'error': '比较处理错误'}), 500
